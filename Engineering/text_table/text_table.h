...
class TextTable {
   public:
    ...
    // enum 使用class的好处：enum class 的类型更安全。https://zhuanlan.zhihu.com/p/21722362
    enum class Align : int { Left, Right, Mid };
    // cpp11 之前一般会提三大函数，无参构造，复制构造，赋值函数。cpp11之后 又加了两个
    // 移动构造，移动赋值，总共有5大函数。如果写一个简单的类还要写这5个函数，太麻烦了，对于自己重新实现了的函数，未实现部分采取default可以省一点事。相对default,还有一个delete关键字，表示不允许这个函数调用。
    TextTable() = default;
    ...
    TextTable& add(const T& value) {
        m_row.values.emplace_back(
            value);  // emplace_back在某些情况下比push_back更搞笑，ref：https://zhuanlan.zhihu.com/p/183861524
